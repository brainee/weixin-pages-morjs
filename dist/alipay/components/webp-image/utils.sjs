...
function isUrl(url) {
    ...
    var urlReg = getRegExp('/[(http(s)?)://(www.)?a-zA-Z0-9@:%._+~#=]{2,256}.[a-z]{2,6}\b([-a-zA-Z0-9@:%_+.~#?&//=]*)/', 'ig');
...
function imageMogr(url, options) {
    ...
    if (!options)
        return url;
    var width = Math.ceil(options.width), height = Math.ceil(options.height), format = options.format, _optionsQuality = options.quality, quality = _optionsQuality === undefined ? 70 : _optionsQuality, _optionsStrip = options.strip, strip = _optionsStrip === undefined ? true : _optionsStrip, crop = options.crop;
